...
| nuxt-gsap-module | 1.7.2 | `$gsap` injection, text extra plugin |
...
**Dead code:** `plugins/scrollmagic.js` registers vue-scrollmagic but ScrollMagic is loaded via CDN script tags in `nuxt.config.js` instead.
...
- `.stack-anim` rows: y: -400 over 2s, stagger 0.3, `slow(0.7, 0.7, false)` ease
...
- `listToShow` recalculated on window resize for responsive row grouping
...
- **No Lenis or native CSS scroll-behavior** configured globally
...
10. **Per-card project animations:** Separate ScrollMagic scenes for similar effects

## Suggested Rebuild Animation Approach

1. **Use GSAP ScrollTrigger** (or Framer Motion `whileInView`) with a single shared context via `@gsap/react`
2. **Hero:** One-time entrance animation on load; static "Fullstack Developer" headline; respect reduced motion
...
5. **Optional:** Lenis for smooth scroll (lightweight, pairs well with GSAP)
6. **Hover:** CSS transitions only for buttons/links
7. **Page transitions:** None needed for one-page portfolio
8. **Loading:** CSS spinner instead of Cloudinary loading GIF on buttons
...
- Use `ScrollTrigger.matchMedia` or a `prefersReducedMotion()` helper
- Keep functional scroll-to-section but with instant or short scroll

## Animation Intent to Preserve (not implementation)

| Old behavior | Rebuild direction |
|--------------|-------------------|
| Hero name typewriter | One-time fade/slide reveal |
| Rotating job titles | Single headline: Fullstack Developer |
| Hero parallax on scroll | Subtle opacity fade or remove |
| Intro line stagger | Shorter paragraph/word reveal |
...
**Library:** GSAP 3.15 + ScrollTrigger via `@gsap/react` (`useGSAP`).
...
| Navigation | Intersection Observer active state (no GSAP) |
| Hero CTA | `MagneticButton` subtle pointer-follow (CSS transform) |

**Reduced motion:** Checked in all animation hooks; global CSS override in `app/globals.css`. Scroll uses `behavior: "auto"` when reduced motion is preferred via `scrollToSection()`.

**Not implemented:** Lenis, infinite hero carousel, ScrollMagic, decorative blob keyframes.
