...
**Gradients:**
- Testimony section: light blue gradient (`#a5ddff` → transparent)
- Footer/section decorations: red and blue ellipse gradients (Cloudinary SVGs)

**Apple-like modernization opportunity:** Shift from multi-accent (yellow + light blue + primary blue) to a restrained neutral palette (whites, grays, single accent) with more negative space and subtler shadows.
...
- **Rounded sections:** `border-radius` 130px on top or bottom corners via `rounded-container` mixin
...
- Utility classes: `mt-2`, `mt-3`, `mb-3`, etc. (likely from missing utilities file)
...
- Hero portrait hidden at `laptop` and below
- Hero `#me-anim` becomes block on mobile-lg
- Stacks section height increases on tablet breakpoints
...
1. **Typography:** Replace dual-font setup with system font stack or single family (SF Pro-like: Inter, Geist, or system-ui)
...
5. **Cards:** Flat or glass-morphism cards with subtle borders instead of heavy shadows and rotation
6. **Motion:** Short, purposeful transitions (300–500ms) vs. long GSAP timelines (5–15s)
7. **Navigation:** Add minimal sticky nav with blur backdrop (iOS-style)
8. **Spacing:** Increase vertical rhythm; use consistent 8px grid via Tailwind
9. **Imagery:** Full-bleed or contained screenshots without skew/rotate
